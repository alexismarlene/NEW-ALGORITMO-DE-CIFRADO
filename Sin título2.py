...
def exito_login():
    ...
    def encriptar():
        ...
        mensaje= Entry(ventana)
        mensaje.pack(fill=X,padx=5,pady=5,ipadx=5,ipady=5)
        msg=mensaje.get()
        password= Entry(ventana)
        password.pack(fill=X,padx=5,pady=5,ipadx=5,ipady=5)
        pwd=password.get()
        
        print( "el mensaje es: ",AESCipher(pwd).encrypt(msg).decode('utf-8'))
    def desencriptar():
        ventana=Toplevel()
        ventana.title("ENCRIPTAR")
        ventana.geometry('380x450')
        entrada=Entry(ventana)
        entrada.pack(fill=X,padx=5,pady=5,ipadx=5,ipady=5)
        ventana.Entry( AESCipher(pwd).encrypt(entrada).decode('utf-8'))
